...
def testDns(parIn=""):
    ...
    try:
        ...
        time.sleep(1)
        page_data1 = currSess.get("https://soloper.pro/embe.php?id=HD1",headers=head)
    ...
    jsonText = jsonText + '{"title":"'+ret+'","myresolve":"wigi@@https://soloper.pro/embe.php?id=HD1",'
...
def wigi(parIn=None):
    ...
    if "nopay.info" in parIn or "nopay2.info" in parIn or "soloper.pro" in parIn:
...
def imdb(parIn):
    ...
    url="https://guardahd.stream/movie/"+parIn
    headers = {
        'user-agent': "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/74.0.3729.131 Safari/537.36"
    }
...
def vudeo(parIn):
    ...
    if PY3:
        page_data = page_data.decode('utf-8')
    logga ("PAGE_VUDEO: "+page_data)
    video_urls = []
    if "The file was deleted" in page_data.replace("\n", "").replace("\r", "").replace("\t", ""):
        src="ignoreme"
        video_urls.append((src, "[COLOR red]FILE WAS DELETED[/COLOR]"))
    else:
