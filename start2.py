...
playlistsFile = os.path.join(home, "playLists.txt")
vDirectoriesFile = os.path.join(addon_data_dir, "virtual_directoriesLists.txt")
tmpListFile = os.path.join(addon_data_dir, 'listaTemp.txt')
favoritesFile = os.path.join(addon_data_dir, 'preferiti.txt')
...
def CategoriesOld():
    ...
    AddDir("[COLOR aqua][B][{0}][/B][/COLOR]".format(getLocaleString(30003)), "favorites", 30, os.path.join(iconsDir, "bright_yellow_star.png"))
    AddDir("[COLOR gold][B]{0}[/B][/COLOR]".format(getLocaleString(30001)), "newList", 20, os.path.join(iconsDir, "NewList.ico"), isFolder=False)
    AddDir("[COLOR gold][B]{0}[/B][/COLOR]".format(getLocaleString(30040)), "newDirectory", 43, os.path.join(iconsDir, "New-folder.png"), isFolder=False)
    ...
    for vDir in vDirs:
        ...
        AddDir("[COLOR lime][B]{0}[/B][/COLOR]".format(vDir["name"]), "{0}".format(y), 44, dir_icon, uuid=vDir["uuid"], isFolder=True)
        y += 1
...
def AddDir(name, url, mode, iconimage='', logos='', index=-1, move=0, uuid='0', isFolder=True, IsPlayable=False, 
    ...
    if mode == 1 or mode == 2:
        ...
        if PY3:
            ...
            urlQuote = quote(url)
        items = [
            (getLocaleString(30009), 'RunPlugin({0}?url={1}&mode=31&iconimage={2}&name={3}&uuid={4})'.format(sys.argv[0], urlQuote, iconimage, name, uuid))
        ]
...
def resetMandraLista():
    ...
    remoteLauncherUrl = "http://test34343.herokuapp.com/test33.php?par=122"
...
def updateMandraLista():
    ...
    if os.path.exists(launcher_file)==True:
        ...
        try:
            ...
            pass
        remoteLauncherUrl = "http://test34343.herokuapp.com/test33.php?par=120"
        strSource = makeRequest(remoteLauncherUrl)
        if strSource is None or strSource == "":
            logga('We failed to get source from '+remoteLauncherUrl)
